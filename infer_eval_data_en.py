...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='math-500', help='math-500, aime24, or aime25')
    ...
    # generate multiple responses for each English run
    logger.info(f"Generating responses for English")
    ...
    if not args.lang_select:
        results_output_path = Path(args.output_dir) / "results" / f"{args.dataset}_en_prefix.jsonl"
    else:
        results_output_path = Path(args.output_dir) / "results" / f"{args.dataset}_en_select.jsonl"
